...
```
`config/base.yaml` and `config/losses/scale_vae.yaml` load automatically. Swap the model config for other backends:
...
### 🔍 Predict / Sampling
Predict commands automatically include `config/base.yaml`, `config/losses/scale_vae.yaml`, and `config/predict.yaml`. Append additional configs to pick the model and custom predict grids:
```bash
